...
import { idTokenSelector, isBookLoadingSelector, tagsSelector } from 'src/store/rootSelector';
import {
	Avatar,
	Paper,
	Button,
...
const Publish = () => {
	...
	const onPublishBook = (idToken: any, book: any) =>
	...
	const setImageHandler = (e: any) => {
		...
		if (images === null) {
			...
			if (e.target.files.length !== 1) {
				...
				setImageAlert(false);
				setImages((state: any) => [...state, ...files]);
				files = [];
			} else {
				setImageAlert(false);
				setImages((state: any) => [...state, e.target.files[0]]);
			}
		}
	};

	const removeImageHandler = (item: any) => {
		const newImages = images.filter((img: any) => img !== item);
	...
	const capitalizeText = (text: string) => {
		return text.replace(/\w\S*/g, (w: string) =>
